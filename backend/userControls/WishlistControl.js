...
const addToWishlist = asyncHandler(async (req, res) => {
    ...
    if (variantsId && userId && proId) {
        let userCart = await db.get().collection(collection.USER_WISHILIST_COLLECTION).findOne({ userId: userId });
        if (userCart) {
            ...
            const insert = await db.get().collection(collection.USER_WISHILIST_COLLECTION).insertOne(cartObj);
            if (insert) {
